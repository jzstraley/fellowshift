...
export function useVacationState({
  useDatabase,
  // Normalized scope (post-migration); both null = fall back to legacy strings
  programId,
  academicYearId,
  fellows,
  schedule,
  ...
  // True once the migration has been run and scope IDs are available
  ...
  // Cached text labels for the normalized scope — used to keep legacy text columns populated
  // during the transition period before they are dropped.
  const [programText, setProgramText] = useState('');
  const [academicYearText, setAcademicYearText] = useState('');

  useEffect(() => {
    if (!useNewScope || !programId || !academicYearId) return;
    let cancelled = false;
    (async () => {
      const [{ data: prog }, { data: year }] = await Promise.all([
        supabase.from('programs').select('name').eq('id', programId).maybeSingle(),
        supabase.from('academic_years').select('label').eq('id', academicYearId).maybeSingle(),
      ]);
      if (!cancelled) {
        if (prog?.name) setProgramText(prog.name);
        if (year?.label) setAcademicYearText(year.label);
      }
    })();
    return () => { cancelled = true; };
  }, [useNewScope, programId, academicYearId]);
  ...
  const fetchRequests = useCallback(async () => {
    ...
    try {
      // ── Fellows ──────────────────────────────────────────────────────────
      // New scope: filter by program_id. Legacy: filter by institution_id.
      const fellowsBaseQ = supabase.from('fellows')
        .select('id, name, pgy_level, program_id, user_id')
        .eq('is_active', true)
        .order('name');
      let { data: fellowsData, error: fellowsErr } = await (
        useNewScope
          ? fellowsBaseQ.eq('program_id', programId)
          : fellowsBaseQ.eq('institution_id', profile.institution_id)
      );
      ...
      if (!fellowsData?.length && userCanApprove) {
        const toInsert = fellows.map(name => ({
          ...
          ...(useNewScope
            ...
            : { institution_id: profile.institution_id, program: profile?.program || '' }),
        }));
      ...
      // ── Block dates ───────────────────────────────────────────────────────
      const blockBaseQ = supabase.from('block_dates')
        .select('id, block_number, start_date, end_date, rotation_number')
        .order('block_number');
      let { data: blockDatesData, error: blockDatesErr } = await (
        useNewScope
          ? blockBaseQ.eq('program_id', programId).eq('academic_year_id', academicYearId)
          : blockBaseQ
              .eq('institution_id', profile.institution_id)
              .eq('program', profile?.program || profile?.institution?.program || '')
      );
      ...
      if ((blockDatesData?.length ?? 0) < expectedBlockCount && userCanApprove) {
        ...
        const toInsert = (localBlockDates || [])
          ...
          .map(b => {
            const sd = new Date(b.start + 'T00:00:00');
            const sy = sd.getFullYear();
            const sm = sd.getMonth() + 1;
            const acadStart = sm >= 7 ? sy : sy - 1;
            const academic_year = `${acadStart}-${acadStart + 1}`;
            return {
              ...
              ...(useNewScope
                ? { program_id: programId, academic_year_id: academicYearId, institution_id: profile?.institution_id, program: programText, academic_year: academicYearText }
                : { institution_id: profile.institution_id, program: profile?.program || profile?.institution?.program || '', academic_year }),
            };
          });
        if (toInsert.length) {
          const conflict = useNewScope
            ? 'program_id,academic_year_id,block_number'
            : 'institution_id,program,academic_year,block_number';
          const { data: seeded, error: seedErr } = await supabase
            .from('block_dates')
            .upsert(toInsert, { onConflict: conflict })
            .select('id, block_number, start_date, end_date, rotation_number');
      ...
      const vrQ = supabase
        ...
        .select(`
          ...
          fellow:fellows!fellow_id (id, name, pgy_level, program_id),
      ...
      const { data: requestsData, error: requestsErr } = await (
        useNewScope
          ? vrQ.eq('program_id', programId).eq('academic_year_id', academicYearId)
          : vrQ
      );
      ...
      const srQ = supabase
        ...
        `)
        .order('created_at', { ascending: false });
      const { data: swapsData, error: swapsErr } = await (
        useNewScope
          ? srQ.eq('program_id', programId).eq('academic_year_id', academicYearId)
          : srQ
      );
  ...
  }, [useDatabase, useNewScope, programId, academicYearId, profile?.institution_id, userCanApprove, fellows, pgyLevels]);
  ...
  const approveDbSwap = async (requestId) => {
    ...
    try {
      ...
      const pulled = await pullCallFloatFromSupabase({
        programId, academicYearId, institutionId: profile?.institution_id,
      });
    ...
    try {
      ...
      if (typeof newDbReq.start_block_id === 'string' && newDbReq.start_block_id.startsWith('local-')) {
        ...
        const toInsert = {
          ...
          ...(useNewScope
            ? { program_id: programId, academic_year_id: academicYearId, institution_id: profile?.institution_id, program: programText, academic_year: academicYearText }
            : { institution_id: profile?.institution_id ?? null, program: profile?.program || profile?.institution?.program || '', academic_year: '' }),
        };
        try {
        ...
        try {
          ...
          const findQ = supabase.from('block_dates').select('id').eq('block_number', newBlockNumber);
          const boundQ = useNewScope
            ? findQ.eq('program_id', programId).eq('academic_year_id', academicYearId)
            : findQ.eq('institution_id', profile?.institution_id ?? null);
          const { data: found, error: findErr } = await boundQ.limit(1);
      ...
      const { error } = await supabase.from('vacation_requests').insert({
        ...
        ...(useNewScope ? { program_id: programId, academic_year_id: academicYearId } : {}),
      });
      if (error) throw error;
  ...
  const submitDayOff = async () => {
    ...
    try {
      ...
      if (matchingBlock) {
        ...
        const dayFindQ = supabase.from('block_dates').select('id').eq('block_number', localMatch.block);
        const boundDayQ = useNewScope
          ? dayFindQ.eq('program_id', programId).eq('academic_year_id', academicYearId)
          : dayFindQ.eq('institution_id', profile?.institution_id);
        const { data: found, error: findErr } = await boundDayQ.limit(1);
